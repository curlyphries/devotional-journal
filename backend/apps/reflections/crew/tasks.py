...
class DailyInsightTask(BaseTask):
    ...
    def _build_context(self, user, reflection) -> dict:
        from ..models import UserJourney, OpenThread
...
class WeeklyReviewTask(BaseTask):
    ...
    def _build_context(self, user) -> dict:
        from ..models import UserJourney, DailyReflection, OpenThread
        ...
        return {
            ...
            'reflection_days': ', '.join(reflection_days) if reflection_days else 'None',
...
class MonthlyRecapTask(BaseTask):
    ...
    def _build_context(self, user) -> dict:
        from ..models import UserJourney, DailyReflection, OpenThread, AlignmentTrend
        ...
        threads_resolved = OpenThread.objects.filter(
            user=user,
            status='resolved',
            resolved_at__date__gte=month_ago
        ).count()
        ...
        for r in reflections:
            ...
            if day_name not in day_scores:
                day_scores[day_name] = []
            avg_score = sum(r.area_scores.values()) / len(r.area_scores) if r.area_scores else 0
        ...
        return {
            ...
            'themes': ', '.join([t[0] for t in top_themes]) if top_themes else 'Various',
            'top_struggles': '; '.join(struggles[:3]) if struggles else 'None noted',
            'growth_areas': ', '.join(journey.focus_areas) if journey else 'Not specified',
            'focus_areas': ', '.join(journey.focus_areas) if journey else 'Not specified',
            'weekly_trends': [{'week': t.period_start, 'averages': t.area_averages} for t in weekly_trends],
            'best_days': sorted_days[0][0] if sorted_days else 'Unknown',
            'challenging_days': sorted_days[-1][0] if len(sorted_days) > 1 else 'Unknown',
            'threads_opened': threads_opened,
            'threads_resolved': threads_resolved,
            'goal': journey.goal_statement if journey else 'Not specified',
            'success_definition': journey.success_definition if journey else 'Not specified',
