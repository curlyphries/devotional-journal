...
def _upsert_study_session(
    *,
    ...
    session = (
        StudyGuideSession.objects.filter(**filters).order_by("-started_at").first()
    )
    if session:
...
class UserJourneyViewSet(viewsets.ModelViewSet):
    ...
    def active(self, request):
        ...
        return Response(
            {"detail": "No active journey"}, status=status.HTTP_404_NOT_FOUND
        )
...
class DailyReflectionViewSet(viewsets.ModelViewSet):
    ...
    def today(self, request):
        ...
        if reflection:
            ...
            return Response(serializer.data)
        return Response(
            {"detail": "No reflection for today"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    def by_date(self, request, date_str=None):
        ...
        if reflection:
            ...
            return Response(serializer.data)
        return Response(
            {"detail": "No reflection for this date"}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    def generate_insight(self, request, pk=None):
        ...
        context = {
            "gratitude": (
                reflection.get_gratitude_note()
                if reflection.encrypted_gratitude
                else ""
            ),
            "struggle": (
                reflection.get_struggle_note() if reflection.encrypted_struggle else ""
            ),
            "intention": (
                reflection.get_tomorrow_intention()
                if reflection.encrypted_intention
                else ""
            ),
...
class AlignmentTrendViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def weekly(self, request):
        """Get weekly trends."""
        trends = (
            self.get_queryset().filter(period_type="week").order_by("-period_start")[:4]
        )
    ...
    def by_area(self, request, area_code=None):
        ...
        trends = (
            self.get_queryset()
            .filter(life_area__code=area_code)
            .order_by("-period_start")[:12]
        )
        serializer = self.get_serializer(trends, many=True)
        return Response(serializer.data)
...
class OpenThreadViewSet(viewsets.ModelViewSet):
    ...
    def needing_followup(self, request):
        ...
        threads = [
            t for t in self.get_queryset().filter(status="open") if t.needs_followup
        ]
        serializer = OpenThreadSerializer(threads, many=True)
        return Response(serializer.data)
    ...
    def respond(self, request):
        ...
        serializer = ThreadResponseSerializer(
...
class CrewView(APIView):
    ...
    def post(self, request, action=None):
        ...
        if action == "weekly-review":
                ...
                "reflections": [
                    {
                        ...
                        "gratitude": (
                            r.get_gratitude_note() if r.encrypted_gratitude else ""
                        ),
                        "struggle": (
                            r.get_struggle_note() if r.encrypted_struggle else ""
                        ),
                    }
            ...
            if result:
                return Response(
                    {"response": result, "agent": agent_name, "provider": "ollama_crew"}
                )
            return Response(
...
class FocusIntentionViewSet(viewsets.ModelViewSet):
    ...
    def active(self, request):
        ...
        intentions = self.get_queryset().filter(
            status="active", period_start__lte=today, period_end__gte=today
        )
    ...
    def today(self, request):
        ...
        todays_passages = DevotionalPassage.objects.filter(
            user=request.user, scheduled_date=today, focus_intention__in=intentions
        )
        ...
        return Response(
            {
                "active_intentions": FocusIntentionSerializer(
                    intentions, many=True
                ).data,
                "todays_passages": DevotionalPassageSerializer(
                    todays_passages, many=True
                ).data,
...
class DevotionalPassageViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def generate_on_demand(self, request):
        ...
        request.data.get("scripture_reference", None)
...
class StudyGuideSessionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def summary(self, request):
        ...
        if session_list:
            avg_progress = round(
                sum(session.progress_percentage for session in session_list)
                / len(session_list),
                1,
            )
...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        active_focus = FocusIntention.objects.filter(
            user=user, status="active", period_end__gte=today
        ).first()
        ...
        active_enrollment = (
            UserPlanEnrollment.objects.filter(user=user, is_active=True)
            .select_related("plan")
            .first()
        )
        ...
        journal_entries_week = JournalEntry.objects.filter(
            user=user, date__gte=week_start
        ).count()
        journal_entries_month = JournalEntry.objects.filter(
        ...
        highlights_today = VerseHighlight.objects.filter(
            user=user, created_at__date=today
        ).count()
        highlights_week = VerseHighlight.objects.filter(
            user=user, created_at__date__gte=week_start
        ).count()

        recent_highlights = (
            VerseHighlight.objects.filter(user=user, note__isnull=False)
            .exclude(note="")
        ...
        reflections_week = DailyReflection.objects.filter(
            user=user, date__gte=week_start
        ).count()
        ...
        latest_trend = (
            AlignmentTrend.objects.filter(user=user).order_by("-created_at").first()
        )
        ...
        if latest_trend and latest_trend.area_averages:
            ...
            for area_name, score in latest_trend.area_averages.items():
                ...
                trend_dir = (
                    "improving"
                    if delta > 0
                    else ("declining" if delta < 0 else "stable")
                )
                life_area_scores.append(
                    {"area": area_name, "score": score, "trend": trend_dir}
                )
...
class ThreadPromptPendingView(APIView):
    ...
    def get(self, request):
        ...
        for thread in threads:
            ...
            recent_prompt = ThreadPrompt.objects.filter(
                thread=thread, created_at__date=today
            ).exists()
            if recent_prompt:
                continue
    ...
    def _generate_prompt_text(self, thread_type: str, days_since: int) -> str:
        ...
        return prompts.get(
            thread_type, f"Checking in on something from {days_since} days ago..."
        )
    ...
    def post(self, request, thread_id):
        ...
        if response_status in ["better", "same", "worse", "resolved"]:
            ...
            ThreadPrompt.objects.create(
                ...
                encrypted_response=encrypt_field(
                    (
                        f"{response_status}: {expanded_text}"
                        if expanded_text
        ...
        if action == "respond":
            ...
            ThreadPrompt.objects.create(
                ...
                encrypted_response=(
                    encrypt_field(response_text, user) if response_text else None
                ),
            )
            thread.followup_count += 1
            thread.status = "progressing"
...
class MilestonesView(APIView):
    ...
    MILESTONE_DEFINITIONS = [
        ...
        },
        {
            ...
            "description": "90-day journal streak",
            "target": 90,
        },
        # Journal milestones
        {
        ...
        # Focus milestones
        {
            ...
            "type": "focus",
            "title": "First Focus",
            "description": "Complete your first focus intention",
            "target": 1,
        },
        {
            "id": "focus_5",
            ...
            "description": "Complete 5 focus intentions",
            "target": 5,
        },
        {
            "id": "focus_12",
            "type": "focus",
            "title": "Year of Focus",
            "description": "Complete 12 focus intentions",
            "target": 12,
        },
        # Reading milestones
        {
    ...
    def get(self, request):
        ...
        focus_completed = FocusIntention.objects.filter(
            user=user, status__in=["completed", "expired"]
        ).count()
        plans_completed = UserPlanEnrollment.objects.filter(
        ...
        journal_dates = set(
            JournalEntry.objects.filter(user=user).values_list(
...
class GrowthDataView(APIView):
    ...
    def get(self, request):
        ...
        if latest_trend and latest_trend.area_averages:
            prev_avgs = (
                previous_trend.area_averages
                if previous_trend and previous_trend.area_averages
                else {}
            )
            for area_name, score in latest_trend.area_averages.items():
        ...
        journal_by_week = dict(
            JournalEntry.objects.filter(user=user, date__gte=eight_weeks_ago)
        ...
        highlight_by_week = dict(
            VerseHighlight.objects.filter(
                user=user, created_at__date__gte=eight_weeks_ago
            )
        ...
        reflection_by_week = dict(
            DailyReflection.objects.filter(user=user, date__gte=eight_weeks_ago)
            .annotate(week=TruncWeek("date"))
            .values("week")
            .annotate(count=Count("id"))
            .values_list("week", "count")
...
class ScriptureInsightView(APIView):
    ...
    def get(self, request):
        ...
        recent_focus = (
            FocusIntention.objects.filter(user=user, status="active")
            .order_by("-created_at")
            .first()
        )

        # Get recent devotional passages
        recent_passages = DevotionalPassage.objects.filter(user=user).order_by(
            "-scheduled_date"
        )[:5]
        ...
        return Response(
            {
                "current_focus": (
                    {
                        "intention": (
                            recent_focus.intention_text if recent_focus else None
                        ),
                        "themes": recent_focus.themes if recent_focus else [],
                        "period_type": (
                            recent_focus.period_type if recent_focus else None
                        ),
                    }
                    if recent_focus
                    else None
