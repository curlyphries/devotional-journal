...
from .models import DevotionalPassage, DevotionalAudit, FocusIntention
from .serializers import DevotionalPassageSerializer
...
class DevotionalAuditViewSet(viewsets.ModelViewSet):
    ...
    def dashboard(self, request):
        ...
        stats = {
            ...
            'average_relevance_score': audits.aggregate(
                avg=Avg('relevance_score')
            )['avg'] or 0,
            'average_user_rating': audits.filter(
    ...
    def flagged_passages(self, request):
        ...
        flagged = self.get_queryset().filter(
...
class DevotionalQualityReportView(APIView):
    ...
    def get(self, request):
        ...
        report = {
            ...
            'generation_stats': {
                ...
                'audit_coverage': (audits.count() / total_generated * 100) if total_generated > 0 else 0
            },
            'accuracy_metrics': {
                'average_scripture_accuracy': audits.aggregate(
                    avg=Avg('scripture_accuracy_score')
                ...
                'passages_below_threshold': audits.filter(
                    Q(scripture_accuracy_score__lt=0.8) |
                    Q(relevance_score__lt=0.5)
                ).count()
            },
            'user_satisfaction': {
                'average_rating': audits.filter(
                    user_rating__isnull=False
    ...
    def _generate_recommendations(self, audits):
        ...
        flagged_ratio = audits.filter(audit_status='flagged').count() / audits.count() if audits.count() > 0 else 0
