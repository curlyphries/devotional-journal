...
Do NOT quote scripture back at him. Speak naturally."""
    ...
    def generate_insight(
        ...
        ]) if area_scores else "No scores provided"
...
"""
    ...
    def generate_study_guide(self, source_type: str, source_context: dict, user_context: Optional[dict] = None) -> dict:
        """
        ...
        return self._fallback_guide(source_type, source_reference, source_context, user_context)
    ...
    def _parse_guide(self, raw_response: str) -> Optional[dict]:
        ...
        return {
            'title': str(payload.get('title', 'Personalized Deep Dive')).strip() or 'Personalized Deep Dive',
            'insight_summary': str(payload.get('insight_summary', '')).strip(),
            'analytical_insights': [str(i).strip() for i in insights[:6] if str(i).strip()],
            'heart_check_questions': [str(q).strip() for q in questions[:6] if str(q).strip()],
            'study_plan': normalized_plan,
            'prayer_focus': str(payload.get('prayer_focus', '')).strip(),
            'next_step': str(payload.get('next_step', '')).strip(),
        }

    def _fallback_guide(self, source_type: str, source_reference: str, source_context: dict, user_context: dict) -> dict:
        """Fallback guide when model generation fails."""
        focus = user_context.get('focus_intention') or 'walking closely with God'
        connection = source_context.get('connection_to_focus') or source_context.get('theme') or 'your current season'
...
Be selective - not every mention is a thread. Only track things that warrant follow-up."""
    ...
    def detect_threads(self, reflection_text: str, struggle_note: str = "") -> list[dict]:
        """
    ...
    def _parse_threads(self, raw_response: str) -> list[dict]:
        ...
        try:
            ...
            valid_threads = []
            valid_types = ['struggle', 'commitment', 'relationship', 'decision', 'confession']
            valid_areas = ['integrity', 'relationships', 'work', 'self_control', 'faith', 'service', 'growth']
...
class ThreadFollowupService:
    ...
    def get_threads_needing_followup(self, user, max_prompts: int = 2) -> list:
        ...
        ).order_by('last_mentioned_at')  # Oldest first
