...
from .models import (
    ...
    OpenThread, ThreadPrompt, FocusIntention, DevotionalPassage, DevotionalAudit,
    StudyGuideSession,
)
...
class LifeAreaSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'scripture_tags', 'reflection_prompts', 'display_order'
        ]
...
class UserJourneySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserJourney
        fields = [
            'id', 'title', 'duration_days', 'started_at',
            'goal_categories', 'goal_statement', 'success_definition',
            'focus_areas', 'specific_struggle',
            'reading_mode', 'reading_plan', 'custom_readings',
            'current_day', 'status', 'completed_at',
            'completion_reflection', 'next_journey',
...
class UserJourneyCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserJourney
        fields = [
            'title', 'duration_days', 'goal_categories',
            'goal_statement', 'success_definition', 'focus_areas',
            'specific_struggle', 'reading_mode', 'reading_plan',
            'custom_readings'
        ]
...
class AlignmentTrendSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'journey', 'period_type', 'period_start', 'period_end',
            'area_averages', 'area_deltas', 'ai_summary', 'patterns',
            'created_at'
        ]
...
class OpenThreadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OpenThread
        fields = [
            'id', 'thread_type', 'summary', 'original_context',
            'related_life_area', 'status', 'created_at',
            'last_mentioned_at', 'last_followup_at',
            'followup_count', 'skip_count', 'days_since_mentioned',
            'needs_followup', 'resolved_at', 'resolution_note'
        ]
        read_only_fields = [
            'id', 'created_at', 'last_mentioned_at', 'last_followup_at',
            'followup_count', 'skip_count', 'resolved_at'
        ]
...
class OpenThreadCreateSerializer(serializers.Serializer):
    ...
    related_life_area = serializers.CharField(max_length=20, required=False, allow_blank=True)
...
class ThreadPromptSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ThreadPrompt
        fields = [
            'id', 'thread', 'thread_summary', 'reflection',
            'prompt_text', 'response', 'expanded', 'expanded_response',
...
class ThreadResponseSerializer(serializers.Serializer):
    ...
    response = serializers.ChoiceField(choices=[
        'better', 'same', 'worse', 'yes', 'no', 'skipped', 'resolved'
    ])
...
class FocusIntentionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'intention_text', 'themes', 'related_life_areas',
            'status', 'passages_generated', 'is_active',
            'passages_count', 'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'themes', 'related_life_areas', 'passages_generated',
            'created_at', 'updated_at'
        ]
...
class FocusIntentionCreateSerializer(serializers.Serializer):
    ...
    period_type = serializers.ChoiceField(choices=['day', 'week', 'month'], default='week')
    ...
    def create(self, validated_data):
        ...
        self._generate_passages(intention, curator, intention_text, period_type, themes, num_passages)
        
        return intention
    
    def _generate_passages(self, intention, curator, intention_text, period_type, themes, num_passages):
...
class DevotionalPassageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'focus_intention', 'sequence_number', 'scheduled_date',
            'scripture_reference', 'scripture_text', 'translation',
            'stylized_quote', 'context_note', 'connection_to_focus',
            'reflection_prompts', 'application_suggestions',
            'is_read', 'read_at', 'user_reflection', 'reflection_saved_at',
            'created_at'
        ]
        read_only_fields = [
            'id', 'focus_intention', 'sequence_number', 'scheduled_date',
...
class DevotionalAuditReviewSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(
        choices=['verified', 'flagged', 'corrected'],
        required=False
    )
