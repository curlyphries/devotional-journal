...
from .models import LifeArea, UserJourney, DailyReflection, AlignmentTrend, OpenThread, ThreadPrompt
...
class UserJourneyAdmin(admin.ModelAdmin):
    list_display = ['user', 'title', 'duration_days', 'current_day', 'status', 'started_at']
...
class OpenThreadAdmin(admin.ModelAdmin):
    list_display = ['user', 'thread_type', 'status', 'days_since_mentioned', 'followup_count', 'created_at']
