...
class VerificationResult:
    ...
    def to_dict(self):
        return {
            ...
            'reference_text_preview': self.reference_text[:200] + '...' if len(self.reference_text) > 200 else self.reference_text,
        }
...
def verify_against_local_kjv(book: str, chapter: int, verse: int, text: str) -> Optional[VerificationResult]:
    """
    Verify text against locally stored KJV.
    Only works for KJV translation.
    """
    from .models import BibleVerse, BibleTranslation
    ...
    try:
        ...
        differences = find_differences(text, local_verse.text) if similarity < EXACT_MATCH_THRESHOLD else []
        
        warnings = []
        if similarity < SIMILARITY_THRESHOLD:
            warnings.append(f"Text differs significantly from local KJV ({similarity*100:.1f}% match)")
...
def verify_against_bible_api(reference: str, text: str) -> Optional[VerificationResult]:
    ...
    differences = find_differences(text, reference_text) if similarity < EXACT_MATCH_THRESHOLD else []
    ...
    if similarity < 0.7:  # Lower threshold for cross-translation
        warnings.append(f"Significant differences from WEB translation ({similarity*100:.1f}% match)")
...
def verify_verse(
    ...
    if results['verifications']:
        ...
        results['overall_verified'] = any(v['verified'] for v in results['verifications'])
...
def verify_chapter(translation: str, book: str, chapter: int, verses: list) -> dict:
    ...
    if results['failed_count'] == 0 and results['verified_count'] > 0:
        results['summary'] = f"✅ All {results['verified_count']} verifiable verses passed integrity check"
    elif results['failed_count'] > 0:
        results['summary'] = f"⚠️ {results['failed_count']} verses have potential integrity issues"
    else:
