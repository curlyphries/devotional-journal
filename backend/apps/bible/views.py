...
from . import bolls_api, text_verifier
...
class PassageReadView(APIView):
    ...
    def get(self, request):
        ...
        verses = BibleVerse.objects.filter(
            translation=translation, book__iexact=data["book"], chapter=data["chapter"]
        )
...
class SearchView(APIView):
    ...
    def get(self, request):
        ...
        verses = BibleVerse.objects.filter(
            translation=translation, text__icontains=data["query"]
        ...
        try:
            ...
            if verse_start and verse_end:
                verses = bolls_api.get_verse_range(
                    translation, book, chapter, int(verse_start), int(verse_end)
                )
            else:
    ...
    def get(self, request):
        ...
        try:
            ...
            if verse:
                ...
                verse_num = int(verse)
                verse_data = next(
                    (v for v in raw_verses if v.get("verse") == verse_num), None
                )
                ...
                return Response(
                    {"type": "single_verse", "source": "bolls.life", **result}
                )
            else:
                ...
                return Response({"type": "chapter", "source": "bolls.life", **result})
