...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    ai_provider = models.CharField(
        max_length=20, choices=AI_PROVIDER_CHOICES, default="none"
    )
    ...
    def set_ai_api_key(self, raw_key: str):
        ...
        if raw_key:
            self.ai_api_key = encrypt_content(raw_key, self.encryption_key_salt).decode(
                "latin-1"
            )
        else:
            self.ai_api_key = ""
    ...
    def get_ai_api_key(self) -> str:
        ...
        try:
            return decrypt_content(
                self.ai_api_key.encode("latin-1"), self.encryption_key_salt
            )
        except Exception:
